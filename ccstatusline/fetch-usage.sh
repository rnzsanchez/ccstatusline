...
if [ -z "$token" ]; then
  # Read OAuth token from the keychain entry Claude Code writes on login
  creds_json=$(security find-generic-password -s "Claude Code-credentials" -w 2>/dev/null)
  if [ -z "$creds_json" ]; then exit 0; fi
...
# ── Fetch ─────────────────────────────────────────────────────────────────────
# user-agent and anthropic-beta mirror what Claude Code sends so the OAuth
